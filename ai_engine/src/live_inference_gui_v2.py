...
import socketio # <<< THƯ VIỆN MỚI ĐỂ NÓI CHUYỆN VỚI NODE.JS
...
SEQ_LENGTH = 100       
FEATURES = 6           
NODEJS_SERVER = 'http://localhost:5000' # <<< PORT CỦA BACKEND ĐÃ DỰNG
...
class GestureNet(nn.Module):
    def __init__(self, num_features=6, num_classes=5, cnn_filters=64, lstm_hidden=128):
        ...
        self.classifier = nn.Sequential(nn.Linear(lstm_hidden * 2, 64), nn.BatchNorm1d(64), nn.ReLU(), nn.Dropout(0.4), nn.Linear(64, num_classes))
    def forward(self, x):
        x = x.permute(0, 2, 1)
        x = self.cnn(x)
        x = x.permute(0, 2, 1)
...
def predict_gesture(model, scaler, label_mapping, device, data_buffer):
    ...
    with torch.no_grad():
        ...
        if confidence_pct > 75.0:
            ...
            current_status = f"❓ KHÔNG CHẮC CHẮN"
            status_color = "gray"
...
def reset_status():
    global current_status, status_color
    current_status = "🟢 SẴN SÀNG - Bấm SPACE"
    status_color = "green"
...
def on_key_press(event):
    ...
    if event.key == ' ' and not is_recording:
        is_recording = True
        recording_buffer =[] 
        current_status = "🔴 ĐANG THU DỮ LIỆU..."
        status_color = "red"
